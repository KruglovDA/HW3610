# Шпаргалка по консольным командам GitHub (Motoyama-DH)

## Общее

Git — распределённая система контроля версий. Что-то вроде возможности сохраняться в компьютерных играх (в Git эквивалент игрового сохранения — коммит). **Важно**: добавление файлов к «сохранению» двухступенчатое: сначала добавляем файл в индекс (командой `git add`), а потом «сохраняем» ( командой `git commit`).
...
**4 основных типа объектов Git**

1. BLOB (сохранность содержимого файлов)
2. Tree (иерархия файловой системы)
3. Commit (ссылка на объект tree)
4. Pack-файлы (система экономичной упаковки объектов [файлов])

**Примечание:** *в Git единицей хранения данных является объект (object), который однозначно определяется т.н. 40-символьным хешем sha1.*
